---
implements: specs/artifacts/prompts/4d-regenerate-context.spec.md
...
**If missing**: Copy `dist/templates/` → `.livespec/templates/` or copy entire `dist/` → `.livespec/` as documented in `specs/strategy/architecture.spec.md` Distribution section.
...
When processing `specs/workspace/patterns.spec.md`, check for "See specs/strategy/" references and include those specs:
- If references `specs/strategy/markdown-standards.spec.md` → Include MSL format, documentation patterns
- If references `specs/strategy/testing-approach.spec.md` → Include TDD workflow
- If references `specs/strategy/coding-standards.spec.md` → Include coding conventions
...
**See**: `specs/features/config/project-config.spec.md` for schema details
