...
```bash
cat specs/artifacts/prompts/registry.spec.md
```
...
7. **audit**
   ...
   - Router logic:
     - `minimalism` → `audit-msl-minimalism.md`
     - `context` → `audit-context-compression.md`
     - `coverage` → `audit-spec-coverage.md`
     - `workspace-specs` → `audit-workspace-specs.md`
     - `workspace-scope` → `audit-workspace-scope.md`
     - `all` → Run all 5 audits sequentially
...
- **Missing `generated_by`** = User customized → Ask before overwriting
