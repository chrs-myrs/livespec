...
**Ask**: "In your own words, what's the difference between LiveSpec's 5 phases (0-DEFINE through 4-EVOLVE) and the numbered folders in specs/ (foundation/, strategy/, features/)?"
...
**Key point**: You might write a spec during **Phase 1 (DESIGN)** but it belongs in **`strategy/`** folder. The phase describes WHEN you work, the layer describes WHAT you're specifying.
...
✅ **PASS indicators**:
- "API JSON" → strategy/ (cross-cutting architectural decision)
- "OAuth auth" → features/ (observable behavior)
- Understands strategy = HOW approach, behaviors = WHAT system does

❌ **FAIL indicators**:
- Both in same folder
- "API JSON" in features/ (too specific for strategy)
- "OAuth auth" in strategy/ (not a strategy, it's a behavior)
...
- **strategy/**: Cross-cutting HOW (architecture, design patterns, tech stack)
  - Example: 'API responses must be JSON' (applies to ALL endpoints)
- **features/**: Observable WHAT (specific features, behaviors)
...
- Check `specs/strategy/layer-definitions.spec.md` for deep architectural understanding"
