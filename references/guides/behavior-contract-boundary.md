...
**Behavior spec** (specs/features/channel-messaging.spec.md):
```markdown
# Channel Messaging
...
**Contract spec** (specs/interfaces/mcp-tools/slack-post.spec.md):
```markdown
...
- `channel` (required): Slack channel ID
  - Type: string
  - Pattern: `^C[A-Z0-9]{8,}$`
  - Behavior: specs/features/channel-messaging.spec.md#channel-resolution

- `text` (required): Message text
  - Type: string
  - Max length: 3000 characters
  - Behavior: specs/features/channel-messaging.spec.md#message-formatting

- `thread_ts` (optional): Thread timestamp for replies
  - Type: string (timestamp)
  - Format: "1234567890.123456"
  - Behavior: specs/features/threading.spec.md#thread-replies
...
**❌ Wrong** (specs/interfaces/api/users.spec.md):
```markdown
# User API Contract
...
**✅ Correct** (specs/interfaces/api/users.spec.md):
```markdown
# User API Contract
...
**Behavior**: specs/features/user-registration.spec.md
```

**Separate behavior spec** (specs/features/user-registration.spec.md):
```markdown
# User Registration
...
**❌ Wrong** (specs/features/authentication.spec.md):
```markdown
# Authentication
...
**✅ Correct** (specs/features/authentication.spec.md):
```markdown
# Authentication
...
**Separate contract spec** (specs/interfaces/api/auth.spec.md):
```markdown
...
**Behavior**: specs/features/authentication.spec.md
```

### Violation 3: Missing Behavior Links

**❌ Wrong** (specs/interfaces/mcp-tools/slack.spec.md):
```markdown
# Slack MCP Tools
...
- `expected_format` (optional): Expected response format
  - Behavior: specs/features/response-validation.spec.md#format-validation
  - Enum: ["multiple_choice", "yes_no", "number"]
```

**Corresponding behavior spec** (specs/features/response-validation.spec.md):
```markdown
# Response Validation
...
**Behavior** (specs/features/threading.spec.md):
...
**Contract** (specs/interfaces/mcp-tools/slack-thread.spec.md):
- `thread_ts` parameter definition
- Type: string (timestamp)
- Format specification
- **Links to**: specs/features/threading.spec.md#thread-replies
...
**Contracts**:
- specs/interfaces/api/auth-rest.spec.md (REST API)
- specs/interfaces/api/auth-graphql.spec.md (GraphQL mutation)
- specs/interfaces/sdk/auth-client.spec.md (SDK function)
...
**Contract** (specs/interfaces/mcp-tools/slack-post.spec.md):
- Single tool definition with multiple parameters

**Behaviors**:
- specs/features/channel-messaging.spec.md (basic posting)
- specs/features/threading.spec.md (thread replies)
- specs/features/response-validation.spec.md (structured responses)
- specs/features/user-resolution.spec.md (username → ID)
...
**Domain Model** (specs/features/domain-models/threading-model.spec.md):
...
## Domain Context
See specs/features/domain-models/threading-model.spec.md for core threading concepts.
...
satisfies:
  - specs/features/messaging.spec.md
---
...
- `channel` (required): Channel ID
  - Behavior: specs/features/messaging.spec.md#channel-resolution
- `text` (required): Message text
  - Behavior: specs/features/messaging.spec.md#message-formatting
```
...
**1. Requirement** (specs/features/thread-communication.spec.md):
```markdown
...
**2. Domain Model** (specs/features/domain-models/threading-model.spec.md):
```markdown
...
**3. Behavior** (specs/features/threading.spec.md):
```markdown
...
## Domain Context
See specs/features/domain-models/threading-model.spec.md
...
**4. Contract** (specs/interfaces/mcp-tools/slack-post.spec.md):
```markdown
# Slack Post Message Contract
...
- `channel` (required): Slack channel ID
  - Type: string
  - Pattern: `^C[A-Z0-9]{8,}$`
  - Behavior: specs/features/threading.spec.md#channel-targeting

- `text` (required): Message text
  - Type: string
  - Max length: 3000
  - Behavior: specs/features/threading.spec.md#message-content
...
**Sequence**:
1. Requirements (WHY) - specs/foundation/
2. **Behaviors (WHAT)** - specs/features/
3. Contracts (INTERFACE) - specs/interfaces/
