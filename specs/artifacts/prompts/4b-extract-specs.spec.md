...
- `specs/interfaces/` directory created (if external interfaces exist)
...
- New `specs/features/[domain]/[feature].spec.md` for extracted behaviors (format: MSL, with confidence markers and extraction metadata if LOW/MEDIUM confidence)
- Updated `specs/interfaces/` for new external interfaces (format: OpenAPI/JSON Schema/AsyncAPI)
