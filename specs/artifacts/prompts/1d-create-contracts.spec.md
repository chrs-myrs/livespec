...
- [!] Prompt actively verifies Phase 0 prerequisites before proceeding, preventing phase jumping.
  ...
  - Checks specs/foundation/outcomes.spec.md exists (if missing → guide to 0c-define-outcomes)
  - Checks specs/foundation/constraints.spec.md exists (if missing → guide to 0f-identify-constraints)
...
- [!] Prompt guides AI agent to create machine-readable contracts (OpenAPI, JSON Schema, AsyncAPI) for all external system interfaces.
  - Produces contract files in `specs/interfaces/` subdirectories (format: OpenAPI 3.0 YAML, JSON Schema draft-07, AsyncAPI 2.0 YAML)
...
- specs/strategy/architecture.spec.md exists (identifies external interfaces)
- specs/features/ exists (defines system behaviors requiring interfaces)
- `specs/interfaces/` directory created with subdirectories (api/, data/, events/)
...
- `specs/interfaces/api/*.yaml` - OpenAPI 3.0 specifications for REST/HTTP APIs (format: YAML with paths, operations, request/response schemas, authentication)
- `specs/interfaces/data/*.json` - JSON Schema draft-07 definitions for data structures (format: JSON with type definitions, validation rules)
- `specs/interfaces/events/*.yaml` - AsyncAPI 2.0 specifications for async messaging (format: YAML with channels, messages, bindings) (if applicable)
...
- Don't duplicate external documentation in `specs/interfaces/`
