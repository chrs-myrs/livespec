...
### Mode Sub-Agent Generation

- [!] Agent generates exactly 3 mode sub-agents in ctxt/
  - Files: define.md, design.md, audit.md
  - Target size per file: 10-15KB
  - NO frontmatter (clean context)
  - Inline edit warning at top
  - Mode-specific methodology content
  - Load triggers matching natural user language
  - Examples and patterns for mode
  - Reference Library section linking to detailed specs
  - Footer: "[Mode] specialist for [Project Name] / Parent: AGENTS.md"
...
- [!] Agent generates exactly 2 utility sub-agents in ctxt/
  - Files: session.md, msl-audit.md
  - Target size per file: 8-12KB
  - NO frontmatter (clean context)
  - Inline edit warning at top
...
- [!] Agent validates complete context tree structure before reporting success
  ...
  - Verifies ctxt/ contains exactly 3 mode files (define.md, design.md, audit.md)
  - Verifies ctxt/domains/ contains expected files based on taxonomy
  - Verifies ctxt/ contains exactly 2 utility files (session.md, msl-audit.md)
...
- [!] Agent provides comprehensive generation report to calling session
  ...
  - Mode sub-agents: 3 files with individual sizes (define, design, audit)
  - Domain sub-agents: Count and sizes
  - Utility sub-agents: 2 files with sizes (session, msl-audit)
  - Total context tree size
  - Typical loaded context size (root + 1 mode)
...
- [!] Agent supports background execution via Task tool with run_in_background parameter
  - Writes progress and results to var/context-generation.log
  - Parent session receives only completion notification
  - No interactive prompts during background execution
  - Errors captured in log file for review
  - Returns summary: "Context regeneration complete. [X]KB generated in [N] files."
...
- [ ] Exactly 3 mode sub-agents generated in ctxt/ (define.md, design.md, audit.md)
- [ ] Domain sub-agents generated based on taxonomy in ctxt/domains/
- [ ] Exactly 2 utility sub-agents generated in ctxt/ (session.md, msl-audit.md)
- [ ] All sub-agents have NO frontmatter
- [ ] All sub-agents have inline edit warning at top
- [ ] All sub-agents within size budgets (10-15KB modes, 6-10KB domains/utils)
